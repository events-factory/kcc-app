...
export default function UploadPage() {
  ...
  const [uploadStep, setUploadStep] = useState<'select' | 'map' | 'review'>(
    'select'
  );
  ...
  const autoMapColumns = (columns: string[]) => {
    ...
    columns.forEach((column) => {
      ...
      if (lowerColumn.includes('badge') || lowerColumn.includes('id')) {
        ...
        lowerColumn.includes('first') ||
        lowerColumn === 'fname' ||
        lowerColumn === 'firstname'
      ) {
        mapping.firstName = column;
      } else if (
        lowerColumn.includes('last') ||
        lowerColumn === 'lname' ||
        lowerColumn === 'lastname'
      ) {
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-500">
            File should contain columns for: Badge ID, First Name, Last Name,
            Email
          </p>
        </div>
      </div>
  ...
  const renderMapStep = () => (
    <div className="rounded-lg border bg-white p-6 shadow-sm">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Match your file columns to the required fields. All fields are
        mandatory.
      </p>
      ...
      <div className="mt-6 flex justify-end space-x-3">
        <Button variant="ghost" onClick={backToSelectStep}>
          Back
        </Button>
        <Button
          variant="primary"
  ...
  const renderReviewStep = () => {
    ...
    return (
      <div className="rounded-lg border bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 flex justify-end space-x-3">
          <Button variant="ghost" onClick={backToMapStep}>
            Back
          </Button>
          <Button
            variant="primary"
